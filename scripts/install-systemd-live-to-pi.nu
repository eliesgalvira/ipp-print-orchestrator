...
use lib/env.nu *
use lib/remote.nu [build-and-copy-nix-closures remote-target run-remote-nu-source run-timed]
...
let runtime_path = ($nix_paths.runtime_path | to nuon)
...
^nu --no-config-file scripts/install-systemd-live-from-pi.nu --runtime-path $runtime_path
"
